...
> **所属阶段**：阶段五 - 企业端运营调度模块
>
> **文档状态**：方案确认（v2 — 与财务结算彻底解耦）
>
> **创建日期**：2026-05-18 ｜ **最近更新**：2026-05-21
>
> **优先级**：高（核心业务闭环）
>
> **关联文档**：
...
| 5 | **已签收**（聚合态） | **由聚合驱动**：任务下所有 active item.status=3 时自动 4→5；外部 `update_status` 不接受 4→5 |
| 7 | 已关闭 | 人工（close 按钮） |
...
- `5 已签收` 不再人工推进；调度工作台「待签收」入口（`status=4` 的任务）触发 item 级签收弹窗，逐行 `updateTaskWaybillItem(status=3)` 后端聚合自动写 `task.status=5`。
...
### 3.3 Waybill.status（数值不变，文案改为客户视角）

| 值 | 名称（新） | 语义 |
|---|------|--------------|
| 0 | 待确认 | 运单刚录入，待运营/客户确认 |
| 1 | 待调度 | 已确认；**未挂入任何活跃任务单** |
| 2 | 调度中 | **至少一行 cargo 被挂入了未取消的任务单**（item.is_deleted=0） |
| 3 | 运输中 | 至少一个挂接行 `item.status ≥ 1` 已装车 |
| 4 | **待签收**（原"已送达"） | **所有未取消台数**对应的 `item.status ≥ 2` 已卸车，且已卸车台数 = `waybill.quantity` |
| 5 | **已签收**（原"已完成"） | **所有未取消台数**对应的 `item.status = 3` 已签收，且已签收台数 = `waybill.quantity` |
| 6 | 已关闭 | 终态（可在任意非草稿状态进入） |

**v2 文案调整原因（2026-05-21）**：
- "已送达" → "待签收"：强调签收是运单维度的客户动作，与任务侧"已到达"语义解耦；
- "已完成" → "已签收"：与 `item.status=3` 对齐，运单层即"客户全部签收完毕，对客户流程闭环"；
- "已调度" → "调度中"：突出"调度过程"而非状态终态。
- **关键澄清**："调度中" = 挂入任务即升档，不必等任务"已派车"。
...
| 5 已签收 | **不再走 propagate**——由 item → task 反向聚合驱动（见 §4.1bis） |
| 7 已关闭 | item 不变 |
| 9 已取消 | 走 `release_all_items_of_task`（释放未签收台数 + 软删 item） |

### 4.1bis Item → Task 聚合（4↔5 双向）

签收下沉到 item 级后，task 4↔5 由 ``TaskWaybillItemService._aggregate_task_status_from_items`` 统一管理：
...
async def _aggregate_task_status_from_items(db, task):
    ...
    if cur == TASK_ARRIVED and all_signed:
        ...
        task.status = TASK_ARRIVED       # 5 → 4（撤销签收联动）
    await db.flush()
```

调用时机：
- `update_item_status` 末尾（item 状态正/反向变更）
- 任何写 item.status 的批量入口（如未来的"撤销签收" item 批量接口）应同样调用
...
target_status = max(
    ...
    4 if unloaded_qty == total_qty and total_qty > 0 else 3,            # 待签收
    5 if signed_qty == total_qty and total_qty > 0 else 4               # 已签收
)
...
- task2 签收 → signed_qty=10 == 10 → waybill 升 5 已签收
...
| 外部 API 推进 `task.status = 5` | 拒绝（4→5 仅由 item 全签收聚合驱动） |
| 外部 API 推进 `task.status = 6` | 拒绝（枚举已下线） |
...
5. **5 已签收 / 7 已关闭 不可走 `revert_status` 直接逆向**：5 必须通过 item 级"撤销签收"反向聚合实现；7 永久锁死，必须建新任务。
...
| 5 | 4 | 客户拒签 / 误签 | **走 item 级"撤销签收"**：单独 ``updateTaskWaybillItem(itemId, status=2)`` 把某行 item 3→2，再由 ``_aggregate_task_status_from_items`` 自动 task 5→4；waybill 同步 5→4 | `operation:task:revert-sign`（按钮挂在 item 行） | 是 |
| 7 | — | 已关闭：**永久不允许任何回退** | — | — | — |
| 9 | — | 已取消：**永久不允许恢复** | — | — | — |
...
_REVERSE_VALID_TRANS = {
    ...
    3: {2},     # 撤回出发
    4: {3},     # 撤回到达
    # 5 → 4：不在此表，由 item 级"撤销签收"反向聚合自动驱动
}
...
| 撤销签收（item 3→2） | 若运单已挂接到下一个新任务，`item.cargo` 重新占用可能失败 → 拒绝撤销并报"已有新挂接占用" |
...
| update_item_status（反向，如撤销签收 3→2）| True |
...
| 4 | 业务 → 财务 | 任务签收（task→5，聚合驱动） → 生成承运商对账候选 / 司机工资候选 | 软触发 | **07 模块实现**，本期不动 |
| 5 | 业务 → 财务 | 运单"已签收 5" → 标记可纳入客户对账 | 软触发 | **07 模块实现**，本期不动 |
...
| 字段/状态 | 含义 | 财务侧使用场景 |
|----------|------|---------------|
| `task.status` | 任务状态 | `=5 已签收` 时可生成承运商对账候选 |
...
| `waybill.status` | 运单状态 | `=5 已签收` 时可生成客户对账候选 |
...
| [task_waybill_item_service.py](../../backend/app/modules/client/services/task/task_waybill_item_service.py) | **新增 `_aggregate_task_status_from_items(task)`** | item 全签收 → task 4→5；撤销签收（item 3→2）且 task=5 → task 5→4；在 `update_item_status` 末尾调用 |
...
3. **部分到达不升 4**：A 拆到 task1 / task2，task1 到达不能让 A 升 4 待签收
4. **全部签收升 5**：task1 + task2 关联的 item 全签收（item.status=3）→ 各 task 自动 4→5 已签收；waybill 在台数总和 = `A.quantity` 时升 5 已签收
...
12. **撤销签收（item 级 3→2）**：撤一行 item，重新占用 `cargo.allocated_quantity`；若该任务下还存在 item.status < 3，task 自动 5→4，waybill 自动 5→4
13. **撤销签收时 cargo 已被新任务挂走**：报"已有新挂接占用"，拒绝撤销
14. **强制取消 3→9**：在途中强制取消，所有 item 释放、未支付费用单自动撤销 4，已支付费用单**保留**并返回提示
15. **5→4 后再 4→5 正向推进**：item 再次签收后聚合再次驱动 task 4→5；备注里能看到完整轨迹
16. **撤销缺少 reason**：返回 422
17. **从 7 已关闭 / 9 已取消 revert**：拒绝
18. **revert 权限校验**：未授权用户调用 → 403
19. **强制取消时 waybill 聚合下降边界**：A 拆到 task1（已在途）+ task2（已签收），task1 强制取消 → 聚合台数变化下 A 应正确处于"运输中 / 待签收 / 已签收"之一（详细分支看签收台数 vs 总台数）
