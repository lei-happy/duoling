...
最终保证：调度员每一次"装车 / 出发 / 到达 / 签收"的动作，能让运单状态自动跟随到对客户可解释的态；线下变更也能在系统里以审计友好的方式回滚。

### 1.2 当前代码评审结论

| # | 问题 | 影响 | 证据 |
|---|------|------|------|
| 1 | **运单状态停在「已确认」** | 调度员把运单挂入任务、装车、签收，运单 status 全程不动 | [waybill_service.update_status](../../backend/app/modules/client/services/waybill/waybill_service.py) L724-L741 无校验直接赋值；rg 搜 `waybill.*status` 在 task 服务下零匹配 |
...
| 4 | **任务推进时挂接行从不同步** | item.status 永远是 0；cargo.allocated_quantity 永远不会随签收释放，"剩余可分配"算不准 | [task_service.update_status](../../backend/app/modules/client/services/task/task_service.py) L613-L638 |
...
- **Task** = 主动状态载体（用户在工作台触发的动作对象，承担"装车/出发/到达/签收"的语义）
...
| Item → Waybill | item 任何写操作（增/删/改/释放） | 调用 `WaybillStatusAggregator.recompute(waybill_id)` |
| Waybill → 任何方向 | 不存在 | 运单不主动驱动任务/挂接 |

> 单向驱动保证了"任何业务侧动作 → 一次完整的同步"，避免循环依赖。

## 三、状态空间规范化

### 3.1 Task.status（保持现有值，不改 schema）

```text
-1 待分配 → 0 待派车 → 1 已派车 → 2 已装车 → 3 在途 → 4 已到达 → 5 已签收 → 6 已结算 → 7 已关闭
                                                                                            9 已取消（终态）
```
...
```text
0 待装车 → 1 已装车 → 2 已卸车 → 3 已签收
```

### 3.3 Waybill.status（**重新定义语义**，数值不变）

| 值 | 名称 | 重新定义后的语义 |
|---|------|--------------|
| 0 | 待确认 | 客户未确认（运单刚录入） |
| 1 | 已确认 | 客户已确认；**未挂入任何活跃任务单** |
| 2 | 已调度 | **至少一行 cargo 被挂入了未取消的任务单**（item.is_deleted=0） |
| 3 | 运输中 | 至少一个挂接行 `item.status ≥ 1` 已装车 |
| 4 | 已送达 | **所有未取消台数**对应的 `item.status ≥ 2` 已卸车，且已卸车台数 = `waybill.quantity` |
| 5 | 已完成 | **所有未取消台数**对应的 `item.status = 3` 已签收，且已签收台数 = `waybill.quantity` |
| 6 | 已取消 | 用户在 status ∈ {0,1} 时手动取消 |

**关键澄清**："已调度" = 挂入任务即升档，不必等任务"已派车"。这与现有「调度员把运单组装成任务单」的语义一致。
...
| 升档 → 4 / → 5 | **min**（终态闸口） | 必须**全部未取消台数都到达 / 都签收**才能升 4 / 5，保护客户对账 |
...
| 5 已签收 | item 2 → 3（已签收）；写 `signed_at`；调用 `_bump_cargo_allocated(-quantity)` 释放占用 |
| 6 已结算 / 7 已关闭 | item 不变 |
| 9 已取消 | 走 `release_all_items_of_task`（释放未签收台数 + 软删 item） |
...
target_status = max(
    ...
    2 if active_count > 0 else 1,
    3 if loaded_qty > 0 else 2,
    4 if unloaded_qty == total_qty and total_qty > 0 else 3,
    5 if signed_qty == total_qty and total_qty > 0 else 4
)
...
升到 4 已送达 / 5 已完成 必须满足"全部未取消台数都到 / 都签"，保护客户对账：

- 一张 10 台运单拆到 task1（6 台）、task2（4 台）
- task1 签收 → 6 台 signed_qty=6 != 10 → waybill 仍为 3 运输中
- task2 签收 → signed_qty=10 == 10 → waybill 升 5 已完成

### 4.4 不允许的正向动作（闸口）

| 闸口 | 规则 |
|------|------|
| `waybill.status ≥ 4` | 不允许新增/编辑/删除任务挂接（运单端已锁状态） |
| `waybill.is_locked = 1` | 已结算/已开票，全锁（财务侧已锁） |
| `task.status ∈ {5,6,7,9}` | item 不能正向改状态 |
...
5. **6 已结算 / 7 已关闭 不可直接逆向**：必须先撤销结算单（财务侧动作），系统再联动把 task 退到 5；7 永久锁死，必须建新任务。
6. **强制取消可越级**：线下取消允许从 2/3/4 任意态直接 → 9，但需要二次确认 + 高权限 + 强制原因。`≥ 5` 状态不允许强制取消（客户已签收，业务上等同完成，应走"差异化处理"流程）。
...
| 5 | 4 | 客户拒签 / 误签 | item 3→2；清空 `item.signed_at`；**重新占用 cargo.allocated_quantity**；waybill 5→4 | `operation:task:revert-sign` | 是 |
| 6 | 5 | **仅通过财务撤销结算单触发**（service 内部联动） | `task.settled_amount` 同步减回 | `operation:task-finance:cancel` | 财务侧已二次确认 |
| 7 | — | 已关闭：**永久不允许任何回退** | — | — | — |
| 9 | — | 已取消：**永久不允许恢复** | — | — | — |
| **2/3/4** | **9** | **强制取消（线下取消）**：释放所有未签收 item；保留承运快照备查；自动撤销关联未支付费用单 | 见 4.5.3 | `operation:task:force-cancel` | 是 |
...
| `cancel_doc`（撤销最终结算单） | 若 `doc_type=3 is_final=1 status 由 3→4` 且 `task.status=6` → 联动 `task.status` 6→5 + waybill 重算（本期补丁，详见 4.5.6） |
| 撤销签收 5→4 | 若运单已挂接到下一个新任务，`item.cargo` 重新占用可能失败 → 拒绝撤销并报"已有新挂接占用" |
...
下列三处现有代码必须在落地时一起改，否则逆向不闭环：

1. **`task_finance_service.cancel_doc` 不联动 task 6→5**：
   - 当前 [L480-L500](../../backend/app/modules/client/services/task/task_finance_service.py) 仅 `_change_status(doc, 4)`
   - 补丁：若被撤销的是 `doc_type=3 and is_final=1`，且 `task.status=6`，需把 `task.status` 退回 5，并触发 waybill 聚合
...
3. **`waybill_service.update_status` 无状态机校验**：
   - 补丁：套上 `WaybillStateMachine.assert_transition`；只允许"人工通道"——`0→1`、`1→6`（手动取消）；`≥ 2` 拒绝（必须通过任务联动）
...
sequenceDiagram
    ...
    U->>API: revert-status target=4 reason="客户拒收"
    API->>TS: revert_status(5->4)
    TS->>TWS: propagate_revert_to_items(task, 5, 4)
    TWS->>TWS: item 3 -> 2 清空 signed_at 重新占用 cargo
    TS->>WAG: recompute_many(allow_downgrade=True)
    WAG->>API: waybill 5 -> 4 已送达
```
...
sequenceDiagram
    ...
    TFS->>TS: 检测 doc_type=3 is_final=1 且 task=6
    TS->>TS: task.status 6 -> 5
    TS->>WAG: recompute_many(allow_downgrade=True)
    WAG->>API: waybill 视全签收情况保留 5 或退 4
```
...
| 撤销签收时 cargo 已被新任务挂走 | 重新占用 cargo 时由 `_bump_cargo_allocated` 抛出"可分配台数不足"，撤销整个事务并报"已有新挂接占用，无法撤销签收" |
...
| 撤销签收时 cargo 已被新挂接占用 | 409 | `已有新挂接占用，无法撤销签收` |
...
| # | 触发方向 | 闸口 | 级别 | 实现位置 |
|---|---------|------|------|---------|
| 1 | 财务 → 业务 | 任务最终结算单已支付 → task 5→6 | 硬联动 | 现有 [`task_finance_service.pay_doc`](../../backend/app/modules/client/services/task/task_finance_service.py) L466-L478（保留） |
| 2 | 财务 → 业务 | 撤销最终结算单 → task 6→5 | 硬联动 | [`task_finance_service.cancel_doc`](../../backend/app/modules/client/services/task/task_finance_service.py)（**本期补丁**） |
| 3 | 业务 → 财务 | 任务强制取消 → 自动撤销该 task 的所有未支付费用单 | 硬联动 | 新增 `cancel_all_unpaid_docs` |
| 4 | 业务 → 财务 | 任务签收（task→5） → 生成承运商对账候选 / 司机工资候选 | 软触发 | **07 模块实现**，本期不动 |
| 5 | 业务 → 财务 | 运单"已完成 5" → 标记可纳入客户对账 | 软触发 | **07 模块实现**，本期不动 |
| 6 | 业务 → 财务 | `waybill.is_locked=1` / `task.status ≥ 6` | 硬闸口（禁止改财务单基准） | **07 模块**在创建/编辑时校验 |
| 7 | 财务 → 业务 | 客户结算单已收款 → waybill 标记 `is_locked=1` | 硬联动 | **07 模块实现** |
| 8 | 财务 → 业务 | 承运商/司机结算单已支付 → task 标记可关闭（→ 7） | 软触发 | **07 模块实现** |

> 闸口 1-3 在本期文档落地（4.5.6 节列入），其余仅声明协议，由 07 模块实现。
...
| 字段/状态 | 含义 | 财务侧使用场景 |
|----------|------|---------------|
| `task.status` | 任务状态 | 5 时可生成承运商对账候选 |
...
| `task.settled_amount` / `prepaid_amount` / `supplement_amount` | 财务聚合冗余 | 报表统计（由 07 模块维护） |
| `waybill.status` | 运单状态 | 5 时可生成客户对账候选 |
| `waybill.freight_amount` | 运费 | 应收基准 |
| `waybill.is_locked` | 锁定 | 已结算/已开票全锁 |
| `item.signed_qty` (派生) | 已签收台数 | 按台对账 |
...
| `backend/app/modules/client/services/waybill/waybill_status_aggregator.py` | 核心 `recompute(db, waybill_id, *, allow_downgrade=False)`、`recompute_many(...)`；用 `SELECT FOR UPDATE` 锁 waybill 行；按 id 升序避免死锁 |
...
| [task_waybill_item_service.py](../../backend/app/modules/client/services/task/task_waybill_item_service.py) | **新增 `propagate_to_items(task, new_status)`** | 按 4.1 表正向同步：2→item 1+loaded_at；4→item 2+unloaded_at；5→item 3+signed_at+释放 cargo |
| [task_waybill_item_service.py](../../backend/app/modules/client/services/task/task_waybill_item_service.py) | **新增 `propagate_revert_to_items(task, old, new)`** | 按 4.5.2 表反向同步：5→4: item 3→2+清 signed_at+**重新占用 cargo**；4→3: item 2→1+清 unloaded_at；2→1: item 1→0+清 loaded_at |
| [task_finance_service.py](../../backend/app/modules/client/services/task/task_finance_service.py) | `pay_doc` | 已有 task 5→6，追加 `recompute_many(allow_downgrade=False)` |
| [task_finance_service.py](../../backend/app/modules/client/services/task/task_finance_service.py) | `cancel_doc` | **本期补丁**：若被撤销的是 `doc_type=3 is_final=1` 且 `task.status=6` → 联动 `task.status` 6→5；聚合 waybill `allow_downgrade=True` |
...
| [waybill_service.py](../../backend/app/modules/client/services/waybill/waybill_service.py) | `update_status` | 套上 `WaybillStateMachine.assert_transition`；只允许"人工通道"：`0→1`、`1→6`；`≥ 2` 拒绝（必须通过任务联动） |
| [waybill_service.py](../../backend/app/modules/client/services/waybill/waybill_service.py) | `delete_waybill` | 增加"无活跃挂接 item"校验 |
| [api/task/task.py](../../backend/app/modules/client/api/task/task.py) | **新增 endpoint** | `POST /{id}/revert-status`、`POST /{id}/force-cancel`（含权限点） |
| [api/waybill/waybill.py](../../backend/app/modules/client/api/waybill/waybill.py) | `update_waybill_status` | 错误提示更具体 |
...
| `schemas/waybill/waybill.py` | `WaybillOut` 新增字段：`hasActiveTaskItems`、`taskCount`、`loadedQuantity`、`unloadedQuantity`、`signedQuantity` |

### 8.4 权限点登记

需在 [seed_client_menus.py](../../backend/scripts/seed/seed_client_menus.py) 的"运输任务单"菜单下新增按钮权限：
...
- `operation:task:revert-sign` 撤销签收
...
| [views/operation/task-workbench/components/action-confirm-sign.vue](../../frontend/client/src/views/operation/task-workbench/components/action-confirm-sign.vue) | 增加 `signedAt` 字段透传后端，与 `item.signed_at` 对应；签收时间不再仅写备注 |
...
- 强制取消警示语：`强制取消将释放所有未签收台数，并撤销 N 张未支付费用单；已支付的 M 张费用单不会自动撤销，请联系财务处理`

## 十、测试要点

### 10.1 正向流程

1. **挂接升档**：新建任务挂入运单 A 的 5 台 → `waybill A.status = 2 已调度`
2. **装车升档**：task → 2 → waybill 升 3 运输中
3. **部分到达不升 4**：A 拆到 task1 / task2，task1 到达不能让 A 升 4
4. **全部签收升 5**：task1 + task2 都到 5 签收，且台数总和 = `A.quantity` → A 升 5
5. **取消任务回退**：A 只挂 task1，task1 取消 → A 退回 1
6. **取消挂接回退**：A 挂在 task1（5 台）+ task2（3 台），task2 整张挂接被删 → A 不变（task1 还在）
7. **结算闸口**：task 已结算后试图修改挂接 → 拒绝
8. **状态机校验**：直接 PUT `/waybill/{id}/status` 跳 0→3 → 422
9. **并发**：两个调度员同时给同一 cargo 行挂接 → 一方失败
10. **回填脚本**：脏数据库执行后所有 `waybill.status` 与挂接事实一致

### 10.2 逆向流程

11. **撤回派车 1→0**：清空 carrier 快照；waybill 仍为 2 已调度（item 还在）
12. **撤销装车 2→1**：item 1→0，`loaded_at` 清空；waybill 3→2
13. **撤销签收 5→4**：item 3→2，重新占用 `cargo.allocated_quantity`；waybill 5→4
14. **撤销签收时 cargo 已被新任务挂走**：报"已有新挂接占用"，拒绝撤销
15. **强制取消 3→9**：在途中强制取消，所有 item 释放、未支付费用单自动撤销 4，已支付费用单**保留**并返回提示
16. **撤销结算单 → task 6→5**：财务侧 `cancel_doc` 一张已支付的最终结算单，task 自动退回 5
17. **5→4 后再 4→5 正向推进**：`actual_arrive_time` / `signed_at` 被新值覆盖，备注里能看到完整轨迹
18. **撤销缺少 reason**：返回 422
19. **从 7 已关闭 / 9 已取消 revert**：拒绝
20. **revert 权限校验**：未授权用户调用 → 403
21. **强制取消时 waybill 聚合下降边界**：A 拆到 task1（已在途）+ task2（已签收），task1 强制取消 → 聚合台数变化下 A 应正确处于"运输中 / 已送达 / 已完成"之一（详细分支看签收台数 vs 总台数）
22. **并发撤销**：同时撤销两张关联同一 waybill 的任务 → 行锁串行化，最终 waybill 状态正确

### 10.3 财务联动

23. **task 5→6**：最终结算单 `is_final=1` 已支付 → task 自动 5→6（**现有逻辑保留**）
24. **task 6→5**：撤销上述结算单 → task 6→5，settled_amount 减回（**本期补丁**）
25. **强制取消联动撤销费用单**：task 强制取消时，未支付费用单全部到 4 已撤销，返回值含已支付数量
