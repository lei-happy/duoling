...
```
0 待派车 → 1 已派车 → 2 已装车 → 3 在途 → 4 已到达 → 5 已签收 → 6 已结算 → 7 已关闭
                                                                          9 已取消（终态）
```

- `status ≤ 1` 时可取消（→9），自动释放 `waybill_cargo.allocated_quantity`
- `status ≥ 6` 不能再修改主表，只能"关闭"（→7）
...
- 结算单 `is_final=1` 且状态变为 `3 已支付` 时，自动把 `task.status` 推进到 `6 已结算`
...
| status | SMALLINT | 是 | 0-待派车 1-已派车 2-已装车 3-在途 4-已到达 5-已签收 6-已结算 7-已关闭 9-已取消 |
...
5. 状态推进：0→1→2→3→4→5→6→7 各节点 + 9 取消的合法跳转校验
...
22. 已支付的结算单（is_final=1）自动把 task.status 推进到 6
