...
> 现有 [`biz_task_finance_doc`](../../backend/app/modules/client/models/task/task_finance_doc.py) 字段集与上表 80% 对齐，剩余字段在 `05.任务级费用单演进` 中渐进补齐。
...
```python
# backend/app/modules/client/services/finance/base/finance_state_machine.py
class FinanceStateMachine:
    @staticmethod
    def assert_transition(doc_kind: str, old: int, new: int, *, has_reason: bool = False) -> None:
        """校验单据状态合法跳转；撤销/退回类必须 has_reason=True"""

    @staticmethod
    def assert_submittable(doc) -> None:
        """0 -> 1 校验：金额>0、必填业务关联字段已填"""

    @staticmethod
    def assert_payable(doc) -> None:
        """2 -> 3 校验：actual_amount、paid_at、pay_method 填写完整"""

    @staticmethod
    def assert_cancellable(doc, *, with_force: bool) -> None:
        """4 撤销校验：终态 5/4/9 不可撤；3 已支付需 with_force=True 且高权限"""

    @staticmethod
    def assert_locked(doc) -> None:
        """is_locked=1 时拒绝任何修改"""

    @staticmethod
    def legal_next(old: int) -> list[int]:
        """UI 联动用：返回当前状态允许跳转的下一步集合"""
```
...
| # | 财务事件 | 反向影响 | 联动类型 | 实现位置 |
|---|---------|---------|---------|---------|
| 1 | 任务级最终结算单已支付（`doc_type=3 is_final=1` → status 3） | `task.status` 5→6 | 硬联动 | 已落地 `task_finance_service.pay_doc` |
| 2 | 撤销已支付的最终结算单 | `task.status` 6→5 | 硬联动 | 已落地 `task_finance_service.cancel_doc`（06 模块本期补丁） |
| 3 | 客户结算单已收款（status 3） | 关联**运单**的 `is_locked=1` | 硬联动 | `02.客户侧` 服务 |
| 4 | 客户发票已开票 / 已作废 | 关联**结算单**的 `is_locked=1 / 0` | 硬联动 | `02.客户侧` 服务 |
| 5 | 承运商结算单已支付 | 关联**任务**的 `is_locked=1`；允许 `task.status` 7 已关闭 | 软触发 | `03.承运商侧` 服务 |
| 6 | 司机工资单已发放 | 关联**任务**记录 `payroll_settled=1`（冗余） | 软触发 | `04.司机工资` 服务 |
...
class FinanceDocEvent(TenantModelBase):
    ...
    __table_args__ = (
        Index("idx_doc_kind_id", "doc_kind", "doc_id"),
...
| 文件 | 内容 |
|------|------|
| `backend/app/modules/client/models/finance/__init__.py` | 包初始化 |
| `backend/app/modules/client/models/finance/finance_doc_base.py` | `FinanceDocBaseMixin` |
| `backend/app/modules/client/models/finance/finance_doc_event.py` | `FinanceDocEvent` |
| `backend/app/modules/client/services/finance/base/finance_state_machine.py` | `FinanceStateMachine` |
| `backend/app/modules/client/services/finance/base/finance_doc_service.py` | 通用 service 模板 |
| `backend/app/modules/client/services/finance/base/finance_doc_event_writer.py` | 写事件的便捷封装 |
| `backend/app/modules/client/services/finance/linkage/task_to_finance.py` | 任务事件 → 财务候选 |
| `backend/app/modules/client/services/finance/linkage/waybill_to_finance.py` | 运单事件 → 财务候选 |
| `backend/app/modules/client/services/finance/linkage/lock_orchestrator.py` | 业务侧 `is_locked` 编排器 |

### 9.2 现有文件改造点

| 文件 | 改动 |
|------|------|
| [`task_finance_service.py`](../../backend/app/modules/client/services/task/task_finance_service.py) | 注入 `FinanceDocEventWriter`；所有状态切换补写事件 |
| [`task_service.update_status`](../../backend/app/modules/client/services/task/task_service.py) | task→5 时调用 `task_to_finance.on_task_signed` 生成候选 |
| [`waybill_service.update_status`](../../backend/app/modules/client/services/waybill/waybill_service.py) | waybill→5 时调用 `waybill_to_finance.on_waybill_completed` |
| [`waybill_service.update_waybill` / `delete_waybill`](../../backend/app/modules/client/services/waybill/waybill_service.py) | 增加 `linkage.waybill_to_finance.assert_unbound(wb_id)` |
| [`task_finance_service.pay_doc`](../../backend/app/modules/client/services/task/task_finance_service.py) | 增加 `lock_orchestrator.lock_task(task_id, by_doc=doc.id)` |
...
| 表 | 迁移 |
|----|------|
| 现有 `biz_task_finance_doc` | 增加 `direction` / `is_locked` / `locked_at` / `locked_by_doc_id` / `idempotency_key` / `period_start` / `period_end`（保持向后兼容，缺省 NULL） |
| 现有 `biz_waybill` / `biz_task` | 增加 `is_locked` / `locked_at` / `locked_by_doc_id`（如已存在则跳过） |
| 新增 `biz_finance_doc_event` | 见 §6.2 |
