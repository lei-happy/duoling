...
flowchart LR
    ...
    Task -.1:1.-> Prepay & Supp & Settle
    Task -.1:1.-> SocPre & SocFinal
    Settle -- "is_final=1 已支付 → task 5→6" --> Task
```

| 项 | 现状 | 评估 |
|----|------|------|
| 表结构 [`biz_task_finance_doc`](../../backend/app/modules/client/models/task/task_finance_doc.py) | 字段集与基座 §01 §2.1 80% 对齐 | 缺：`direction` / `is_locked` / `locked_at` / `locked_by_doc_id` / `idempotency_key` / `period_start` / `period_end` |
| 状态机 [`task_finance_service`](../../backend/app/modules/client/services/task/task_finance_service.py) L42-L49 | 与基座 §01 §2.2.3 任务级行完全对齐 | OK |
| `pay_doc` 联动 task 5→6 | 已实现 | OK |
| `cancel_doc` 联动 task 6→5 | 06 模块本期补丁已落地 | OK |
| `cancel_all_unpaid_docs` | 06 模块本期已落地 | OK |
| 审计事件流 | 缺失 | 需对接 `biz_finance_doc_event` |
| `is_locked` 联动 | 缺失 | 需对接 `lock_orchestrator` |
| 幂等 | 缺失 | 需加 `idempotency_key` |
...
| 任务级最终结算单 doc_type=3 `is_final=1` | 单任务最终结清，触发 task 5→6 | 与"承运商对账"互斥（一张任务二选一） |
...
| `services/task/task_finance_service.pay_doc` | 2→3 写事件（6-支付）；调用 `lock_orchestrator.lock_task_if_final(doc)` 若 `doc_type=3 is_final=1` 锁定任务 |
| `services/task/task_finance_service.cancel_doc` | 任意→4 写事件（8-撤销 或 9-强制撤销）；已落地 task 6→5 联动 |
...
5. **`is_final=1` 已支付**：task 5→6（已有）；同时 task `is_locked=1`、`locked_by_doc_id=doc.id`
6. **撤销该结算单**：task 6→5（已有）；task `is_locked=0`、`locked_by_doc_id=NULL`
7. **`is_final=0` 已支付**：task **不变** 5；锁定**不**触发
