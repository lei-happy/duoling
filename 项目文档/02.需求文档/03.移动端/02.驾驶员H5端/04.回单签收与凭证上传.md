...
> **术语澄清（与企业端"回单"区分）**：本文档"回单签收"指**司机端的签收动作**（item `0/1/2 → 3`，并可上传底单照片）。企业端另有**运单维度的"确认回单"**——运单全量签收后把签收底单返还货主，将 `waybill.status 5 已签收 → 6 已回单`，凭证落 `biz_waybill_receipt` 表，由 `WaybillReceiptService` 承载。两者层级不同：司机签收驱动 item/任务聚合；企业端回单是运单侧人工动作，与任务状态机独立。详见 [06.运营调度模块/02.运单与任务单状态机联动设计](../../02.企业端/06.运营调度模块/02.运单与任务单状态机联动设计.md) §3.5。
