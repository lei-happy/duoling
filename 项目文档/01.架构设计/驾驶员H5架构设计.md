...
│   ├── auth.py                 # /auth/*
│   ├── task.py                 # /task/*
│   ├── task_receipt.py         # /task-receipt/*
│   ├── finance.py              # /finance/*
│   └── profile.py              # /profile/*
├── schemas/
│   ├── __init__.py
│   ├── task.py                 # Driver* 入参 / 出参
│   ├── finance.py
│   └── profile.py
└── services/
    ├── __init__.py
    ...
    ├── driver_task_service.py  # 薄层包装：confirm-load / depart / confirm-arrive / sign
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/my` | 我的任务分页（`status` / `keyword` 过滤） |
| GET | `/{id}` | 任务详情（segments + items） |
| POST | `/{id}/confirm-load` | 1→2 已装车（创建装车记录 event=1 聚合） |
...
### 5.3 回单 `/api/driver/task-receipt`

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/upload` | 上传回单图片（v0.1 占位，v0.2 落表） |
| GET | `/my` | 我的回单列表（v0.1 返回空，v0.2 启用） |
