...
| 意见反馈 | ✅ | `/api/console/feedback` | 用户之声/意见反馈 | Client 提交 + Console 处理；数据在平台库 `sys_feedback` |
...
智途移动端遵循"**先 H5 → 后小程序 → 后原生 APP**"的演进路径，所有形态
共享同一份 `/api/driver/*` 接口与状态机；前端壳替换不影响业务层。

| 形态 | 状态 | 目录 | 说明 |
|------|------|------|------|
| 驾驶员 H5 | 🔧 | `frontend/driver-h5` | 本期重点：Vue 3 + Vant 4 + Vite，端口 5176；支持登录、多企业切换、任务流转、收入查询、个人中心 |
| 驾驶员微信小程序 | 📋 | （远期） | 计划用 UniApp 复用 H5 业务层，外壳替换 |
| 驾驶员原生 APP | 📋 | （远期） | 计划用 Capacitor 包壳，支持位置上报 / 电子回单 / 推送 |
| 管理人员小程序 | 📋 | （远期） | 移动审批 / 报表查看 / 消息通知 |

> 驾驶员 H5 端详见 [驾驶员H5架构设计.md](驾驶员H5架构设计.md) 与
> [doc/02.需求文档/03.移动端/02.驾驶员H5端/](../02.需求文档/03.移动端/02.驾驶员H5端/)
...
- **console**：管理后台模块。操作平台主库（zt_platform）。
  ...
  - 部分实现：组织架构（stub）
  - 已实现：意见反馈列表/详情/处理（`/api/console/feedback`）
  - 待实现：操作日志查询、登录日志查询
...
| /api/driver/* | 驾驶员移动端接口（H5 / APP / 小程序共用） | JWT（`user_type=3`，必带 `tenant_code`） | zt_biz_{code} |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/auth/login` | 客户端登录（手机号 + 密码，支持多租户选择） |
| GET | `/auth/user-info` | 获取当前用户信息（含角色、菜单、权限） |
| PUT | `/auth/password` | 修改密码（支持首次登录强制修改） |
| GET | `/auth/user-tenants` | 获取当前用户可切换的租户列表 |
| POST | `/auth/switch-tenant` | 切换租户（签发新 Token） |
...
| 驾驶员 H5 | frontend/driver-h5 | 5176 | `/api/driver` | 驾驶员移动端（Vue 3 + Vant 4，轻量级独立项目） |
...
1. 用户提交手机号（phone）+ 密码（Console 端直接登录；Client 端还需租户编码，支持多租户选择）
2. 后端验证后签发 JWT Token
3. Token 载荷包含：user_id、phone、user_type、tenant_code、roles
4. 前端将 Token 存储在 localStorage，每次请求通过 `Authorization: Bearer {token}` 发送
5. 响应头中若包含新 Token，前端自动更新（Token 刷新机制）
6. Client 端支持登录后切换租户（签发新 Token）

> 详细账号体系设计参见 [账号体系设计.md](账号体系设计.md)
...
- `/api/console/auth/login`、`/api/console/auth/refresh`
- `/api/client/auth/login`、`/api/client/auth/refresh`
- `/api/driver/auth/login`、`/api/driver/auth/sms-login`、`/api/driver/auth/refresh`
- `/api/open/*`
