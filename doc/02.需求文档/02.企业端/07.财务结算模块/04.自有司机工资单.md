...
> **所属阶段**：阶段六 - 财务结算模块
>
> **文档状态**：已落地（第 3 期：工资单 + 工资项 + 发放登记 前后端完成）
>
> **创建日期**：2026-05-19
>
> **最近更新**：2026-08-14（第 3 期落地回写：§7.1 页面标已落地、§7.2 换成实际组件清单）
>
> **优先级**：中高（影响自有车队薪资发放周期）
>
> **关联文档**：
...
| 路径 | 页面 | 菜单 id | 状态 |
|------|------|:------:|:---:|
| `/finance/driver-payroll` | 司机工资单列表 + 详情抽屉 | 955 | ✅ |

新建工资单不占独立路由，走列表页内的弹窗（司机 + 周期 + 任务候选），与客户 / 承运商侧口径一致。

### 7.2 关键组件（第 3 期已落地）

| 组件 | 用途 |
|------|------|
| `driver-payroll/components/payroll-create.vue` | 建单：选司机与周期 + 勾选候选任务 |
| `driver-payroll/components/payroll-add-tasks.vue` | 草稿态追加任务 |
| `driver-payroll/components/payroll-task-adjust.vue` | 单任务提成调整 |
| `driver-payroll/components/payroll-item-edit.vue` | 工资项编辑（按 `category` 分应发 / 扣减 / 抵账，带常用项预设） |
| `driver-payroll/components/payroll-detail.vue` | 详情抽屉：任务提成、工资项、事件流 + 工资条视图 |
| `driver-payroll/components/payroll-pay.vue` | 发放登记（选司机账户 + 凭证） |

工资条没有单独的预览组件，直接放在详情抽屉的一个 Tab 里——它读的就是详情已有的数据，独立组件只会多一层透传。
