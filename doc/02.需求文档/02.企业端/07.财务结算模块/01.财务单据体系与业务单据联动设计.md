...
| `services/finance/base/finance_doc_service.py` | `FinanceDocService` 通用单据动作模板（单号 / 状态机 / 事件 / 按钮位） | ✅ 第 1 期；任务级费用单出于向后兼容仍内联在 `task_finance_service`，新单据一律继承本模板 |
| `services/finance/linkage/task_to_finance.py` | 应付侧候选池、扣减额口径、互斥、置脏文案 | ✅ 第 1 期 |
| `services/finance/linkage/waybill_to_finance.py` | 客户侧候选池、签收台数口径、编辑拦截、置脏文案 | ✅ 第 1 期 |
| `models/finance/recon_diff.py` + `services/finance/recon/` | 对账差异表与一致性核对器（见 `09`） | ✅ 第 1 期 |
...
| [`task_service`](../../../../backend/app/modules/client/services/task/task_service.py) | 成本 / 挂接 / 状态变更处调 `TaskToFinance.on_*_changed` 置脏 | 🕒 第 3 期接入（函数已就绪） |
| [`waybill_service`](../../../../backend/app/modules/client/services/waybill/waybill_service.py) | 改删前 `WaybillToFinance.assert_unbound`；运费 / 台数 / 状态变更处置脏 | 🕒 第 2 期接入（函数已就绪） |

> 候选池是**查询派生**的（`list_candidates` 实时按状态与挂接关系过滤），不需要业务侧在 `→5` 时回调写标记。原设计的 `on_waybill_completed` / `on_task_signed` 因此取消。
