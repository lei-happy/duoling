...
> **所属阶段**：阶段六 - 财务结算模块
>
> **文档状态**：已落地（第 3 期：对账单 + 结算单 + 付款登记 前后端完成）
>
> **创建日期**：2026-05-19
>
> **最近更新**：2026-08-14（第 3 期落地回写：§8.1 两个页面标已落地并记录路由差异、§8.2 换成实际组件清单、§10.1 补实际落点）
>
...
| 路径 | 页面 | 菜单 id | 状态 |
|------|------|:------:|:---:|
| `/finance/carrier-recon` | 承运商对账单列表 + 详情抽屉 | 948 | ✅ |
| `/finance/carrier-settlement` | 承运商结算单列表 + 详情抽屉 | 949 | ✅ |

> **与原设计的两处差异**：
>
> 1. **结算单路由用 `/finance/carrier-settlement` 全拼**，与菜单一致；API 前缀仍是 `carrier-settlement`。
> 2. **新建对账 / 新建结算不占独立路由**，`/create` 子路由取消，改为列表页内的弹窗候选选择器——与客户侧口径一致，理由见 [02](02.客户侧-对账与结算与发票.md) §8.1。

### 8.2 关键组件（第 3 期已落地）

| 组件 | 用途 |
|------|------|
| `carrier-recon/components/carrier-recon-create.vue` | 建单：选承运商与周期 + 勾选候选任务（含预付/补款提示） |
| `carrier-recon/components/carrier-recon-add-tasks.vue` | 草稿态追加任务 |
| `carrier-recon/components/carrier-recon-line-adjust.vue` | 对账行调整：数量 / 单价 / 扣减 / 调整额 |
| `carrier-recon/components/carrier-recon-detail.vue` | 详情抽屉：对账行（含扣减与调整列）、事件流 + 动作区 |
| `carrier-recon/components/carrier-recon-sign.vue` | 承运商回签登记 |
| `carrier-settlement/components/carrier-settlement-create.vue` | 建单：勾选已确认对账单 |
| `carrier-settlement/components/carrier-settlement-detail.vue` | 详情抽屉：关联对账单、付款构成、事件流 |
| `carrier-settlement/components/carrier-settlement-pay.vue` | 付款登记（选承运商结算账户，校验账户启用） |

承运商结算账户选择器没有单独成组件，直接在付款弹窗里按 `carrierId` 拉账户下拉——只有一个使用方。
...
第 3 期已全部落地。`_doc` 后缀只保留在模型与服务上（`models/finance/carrier_settlement_doc.py`、`services/finance/carrier/carrier_settlement_doc_service.py`），用于与承运商档案里的 `CarrierSettlement`（结算账户）区分；`schemas/finance/carrier_settlement.py` 与 `api/finance/carrier_settlement.py` 不带后缀，因为 DTO 与路由层不存在撞名。
