...
| 4 | 待签收 | **所有未取消台数**对应的 `item.status ≥ 2` 已卸车，且已卸车台数 = `waybill.quantity` | 聚合（min 闸口） |
| 5 | 已签收 | **所有未取消台数**对应的 `item.status = 3` 已签收，且已签收台数 = `waybill.quantity` | 聚合（min 闸口） |
| 6 | **已回单**（v3 新增） | 签收底单已返还货主（人工动作），运单对货主的真正闭环 | **人工**（回单 API） |
...
- "已送达" → "待签收"、"已完成" → "已签收"、"已调度" → "调度中"；关键："调度中" = 挂入任务即升档，不必等"已派车"。

**v3 调整原因（2026-06-23）**：新增 `6 已回单` 表达"签收底单返还货主"，原 `6 已关闭` 整体后移为 `7`（保持数值单调，便于聚合器 max 比较）；存量 `status=6` 关闭数据迁移为 `7`。
...
```text
5 已签收 --确认回单(人工)--> 6 已回单 --撤销回单(人工)--> 5 已签收
6 已回单 --关闭(人工)--> 7 已关闭
```

| 动作 | 状态跳转 | 落库 | 约束 |
|------|---------|------|------|
| 确认回单 | 5 → 6 | 落一条 `biz_waybill_receipt`（底单文件/回收时间/操作人）+ 写 `waybill.receipt_at` | 仅 `5 已签收` 可确认 |
| 撤销回单 | 6 → 5 | 软删该运单回单凭证 + 清 `receipt_at` | 仅 `6 已回单` 可撤销（`7 已关闭` 后不可撤销） |

**独立性防护**：item 级「撤销签收」（item 3→2）时，若关联运单已进入 `6 已回单`，则**拦截**并提示"运单已回单，请先在运单侧撤销回单后再撤销签收"——避免底单已交付货主后又被任务侧反向破坏。

**与任务的关系**：1 个任务挂 3 张运单，其中 1 张已回单、2 张已签收时，任务状态**不变**（仍按自身状态机），任务列表/详情仅**只读展示**这组运单状态分布（部分回单 / 全部回单由用户从运单状态自行解读）。
...
| 闸口 | 规则 |
|------|------|
| `waybill.status ≥ 4` | 不允许新增/编辑/删除任务挂接（运单端已锁状态；含 4 待签收 / 5 已签收 / 6 已回单 / 7 已关闭） |
...
| item 撤销签收（3→2）且关联运单 `status ≥ 6 已回单` | 拒绝（须先在运单侧撤销回单） |
...
| `waybill.status` | 运单状态 | `=5 已签收`/`=6 已回单` 时可生成客户对账候选 |
...
| [task_waybill_item_service.py](../../../../backend/app/modules/client/services/task/task_waybill_item_service.py) | `update_item_status` | **v3 防护**：item 从 `3 已签收` 反向且关联运单为 `6 已回单` 时抛 `BizException`，要求先撤销回单 |
...
| `views/operation/task/components/waybill-status-summary.vue` | **新建**：只读展示运单状态分布（如 `已签收×2 已回单×1`），供任务列表/详情复用 |
