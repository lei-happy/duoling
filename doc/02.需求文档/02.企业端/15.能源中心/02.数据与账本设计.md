...
>
> 本文回答：19 张表怎么分域、余额为什么只能由流水改、消费如何去重与匹配、迁移和线上运维看什么。
>
...
| 3 | `biz_energy_product` | 能源商品 | 主数据 | CRUD |
| 4 | `biz_energy_vehicle_profile` | 车辆能源档案 | 主数据 | 按 `vehicle_id` upsert |
| 5 | `biz_energy_account` | 能源账户 | 账户介质 | 余额字段禁止业务直接 UPDATE |
| 6 | `biz_energy_card` | 能源卡 | 账户介质 | CRUD |
| 7 | `biz_energy_card_binding` | 卡绑定历史 | 账户介质 | 只追加区间，解绑写 `end_time` |
| 8 | `biz_energy_account_txn` | 资金流水 | 资金账本 | **append-only** |
| 9 | `biz_energy_recharge` | 充值单 | 资金账本 | 单据状态机 |
| 10 | `biz_energy_account_daily_snapshot` | 每日余额快照 | 资金账本 | worker 按日 upsert |
| 11 | `biz_energy_consumption_raw` | 消费原始层 | 消费 | 只增；`data_hash` 唯一 |
| 12 | `biz_energy_consumption` | 标准消费流水 | 消费 | 标准化后写入 |
| 13 | `biz_energy_connector` | 连接器实例 | 接入 | 含映射与游标 |
| 14 | `biz_energy_sync_task` | 同步任务队列 | 接入 | 形态对齐 `biz_cost_calc_task` |
| 15 | `biz_energy_recon` | 对账单 | 对账风控 | 单据 |
| 16 | `biz_energy_recon_item` | 对账明细 | 对账风控 | 随对账单生成 |
| 17 | `biz_energy_rule` | 风控规则 | 对账风控 | 阈值可配 |
| 18 | `biz_energy_exception` | 异常记录 | 对账风控 | 形态对齐成本计算异常 |
| 19 | `biz_energy_cost_allocation` | 成本归集 | 归集 | worker 增量刷新 |
