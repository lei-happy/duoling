...
DICT_DEFS = [
    ...
    # 司机结算模式（驱动费用单形态：承包制→承包单；统一管理→预付/补款/结算；计件→预留工资单）
    ("driver_settlement_mode", "司机结算模式", 26, [
        ("承包制", "1", 0),
        ("统一管理", "2", 10),
        ("计件提成", "3", 20),
    ]),
    # ===== 运输任务单 / 任务单财务相关字典 =====
    ("expense_type", "费用项类型（任务单财务）", 30, [
        ("油费", "oil", 0),
        ("过路费", "toll", 10),
        ("装卸费", "loading", 20),
        ("停车费", "parking", 30),
    ...
    ("pay_method", "支付方式（任务单财务）", 32, [
        ("银行转账", "bank_transfer", 0),
        ("油卡", "fuel_card", 10),
        ("油气款", "fuel_gas", 20),
        ("现金", "cash", 30),
        ("微信", "wechat", 40),
    ...
    ("carrier_cost_type", "承运成本类型（任务单）", 34, [
        ("包车", "charter", 0),
        ("按台", "per_unit", 10),
        ("按吨公里", "per_ton_km", 20),
        ("其他", "other", 90),
