...
class TenantVehicleSeriesService:
    ...
    async def page_series(
        ...
        exists = await db.execute(
            select(BizVehicleBrand.brand_id).where(
                BizVehicleBrand.brand_id == brand_id
            )
        )
        if exists.scalar_one_or_none() is None:
            raise BizException("品牌不存在")

        base = select(BizVehicleSeries).where(
            BizVehicleSeries.brand_id == brand_id
        )
        if keyword:
            base = base.where(BizVehicleSeries.series_name.contains(keyword.strip()))

        count_q = select(func.count()).select_from(base.subquery())
        count = (await db.execute(count_q)).scalar() or 0

        result = await db.execute(
            base.order_by(BizVehicleSeries.series_id)
            .offset((page - 1) * limit)
            .limit(limit)
        )
    ...
    async def create_series(
        ...
        exists = await db.execute(
            select(BizVehicleBrand.brand_id).where(
                BizVehicleBrand.brand_id == data.brandId
            )
        )
        if exists.scalar_one_or_none() is None:
            raise BizException("品牌不存在")
        row = BizVehicleSeries(
            ...
            approach_angle=TenantVehicleSeriesService._to_decimal(data.approachAngle),
            departure_angle=TenantVehicleSeriesService._to_decimal(data.departureAngle),
    ...
    async def update_series(
        ...
        if data.approachAngle is not None:
            row.approach_angle = TenantVehicleSeriesService._to_decimal(
                data.approachAngle
            )
        if data.departureAngle is not None:
            row.departure_angle = TenantVehicleSeriesService._to_decimal(
                data.departureAngle
            )
