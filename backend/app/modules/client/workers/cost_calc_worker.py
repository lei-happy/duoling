"""
成本计算 Worker（多租户轮询，与收入侧 FreightCalcWorker 对称）

部署形态：
  - 推荐：独立 docker service（入口 app/workers/cost_calc_main.py）。
  - 兼容：环境变量 COST_CALC_WORKER_ENABLED=1 时也能在 API 进程内嵌启动
    （仅本地开发 / 单实例场景，多 uvicorn worker 生产环境禁止打开）。

实现要点：
  - APScheduler 定时（默认 5s）扫所有 db_initialized=1 的租户库，
    认领并执行 biz_cost_calc_task。
"""
...
class CostCalcWorker:
    ...
    def __init__(self) -> None:
        ...
        self._interval_sec = int(os.getenv("COST_CALC_WORKER_INTERVAL", "5"))
        self._batch_size = int(os.getenv("COST_CALC_WORKER_BATCH", "20"))
        self._tenants_missing_table: set[str] = set()

    @staticmethod
    def _read_enabled() -> bool:
        v = os.getenv("COST_CALC_WORKER_ENABLED", "0").strip().lower()
    ...
    def start(self) -> None:
        if not self._enabled:
            logger.info(
                "[CostCalcWorker] API 进程内未启动（COST_CALC_WORKER_ENABLED!=1）；"
                "请确认独立 cost-worker 容器已运行"
            )
            return
        self._do_start()
    ...
    def _do_start(self) -> None:
        ...
        logger.info(
            f"[CostCalcWorker] 已启动，间隔 {self._interval_sec}s，"
    ...
    def shutdown(self) -> None:
        if self._scheduler:
            try:
                ...
                logger.warning(f"[CostCalcWorker] shutdown 异常: {e}")
            self._scheduler = None
            logger.info("[CostCalcWorker] 已停止")
    ...
    async def _tick_safely(self) -> None:
        ...
        async with self._running_lock:
            try:
                ...
                logger.exception(f"[CostCalcWorker] tick 异常: {e}")
            ...
            try:
                ...
                if "1146" in msg or "doesn't exist" in msg:
                    if code not in self._tenants_missing_table:
                        ...
                        logger.warning(
                            f"[CostCalcWorker] 跳过租户 {code}：成本引擎表缺失（"
                            f"未开通 billing_cost_rule 功能？）。后续轮询将静默跳过。"
                        )
                else:
                    logger.warning(f"[CostCalcWorker] 处理租户 {code} 失败: {e}")
    ...
    async def _process_tenant(self, tenant_code: str) -> None:
        ...
        async with factory() as session:
            tasks = await CostCalcTaskService.claim_pending(
    ...
    async def _run_one_task(self, session: AsyncSession, task) -> None:
        try:
            if task.target_type == "task":
                await TaskCostCalcService.calculate_and_persist(
                    session,
                    task_id=task.target_id,
                    triggered_by=task.task_type,
                    triggered_user_id=task.triggered_by_user_id,
                )
                await CostCalcTaskService.mark_success(session, task.id)
            else:
                await CostCalcTaskService.mark_failed(
                    session, task.id,
                    f"unsupported target_type: {task.target_type}",
                )
        except Exception as e:  # noqa: BLE001
            logger.warning(
                f"[CostCalcWorker] task#{task.id} (task={task.target_id}) 执行失败: {e}"
            )
            try:
                await CostCalcTaskService.mark_failed(session, task.id, repr(e))
            except Exception:
                logger.exception("[CostCalcWorker] mark_failed 二次失败")


# 全局单例
cost_calc_worker = CostCalcWorker()


def setup_worker_with_settings() -> None:
    _ = get_settings()
    cost_calc_worker.start()
