...
from app.modules.client.services.billing.cost_exception_service import (
    CostExceptionService,
)
...
async def page_tasks(
    ...
    data = await CostCalcTaskService.page_tasks(
...
@operation_log(module="成本引擎", action="重试任务", description="重置成本计算任务为 pending")
async def retry_task(
    ...
    await CostCalcTaskService.retry_task(db, calc_task_id)
    return success()
...
async def page_exceptions(
    ...
    data = await CostExceptionService.page_exceptions(
        db, page=page, page_size=page_size,
...
async def stats_exceptions(
    ...
    data = await CostExceptionService.stats(db)
    return success(data=data)
...
@operation_log(module="成本引擎", action="处理异常", description="标记成本异常为已处理")
async def resolve_exception(
    ...
    await CostExceptionService.resolve(
...
@operation_log(module="成本引擎", action="忽略异常", description="标记成本异常为已忽略")
async def ignore_exception(
    ...
    await CostExceptionService.ignore(
        db, exception_id, user_id=current_user.user_id, remark=payload.remark,
    )
    return success()
...
@operation_log(module="成本引擎", action="批量重算异常", description="批量对异常关联任务重算")
async def batch_recalc_exceptions(
    ...
    data = await CostExceptionService.batch_recalc(
