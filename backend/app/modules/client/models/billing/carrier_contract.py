...
与客户收入侧 biz_freight_contract 对称：以「承运商」为核心维护承运价合同，
派车给承运商（carrier_type=2）后据此自动计算整单运费。
"""
...
class CarrierContract(TenantModelBase):
    """承运商合同"""
    __tablename__ = "biz_carrier_contract"
    __table_args__ = {"comment": "承运商合同表"}
    ...
    carrier_id: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="承运商ID（biz_carrier.id）"
    )
    carrier_name: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="承运商名称"
    )
