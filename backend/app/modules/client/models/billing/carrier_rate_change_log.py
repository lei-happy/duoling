...
class CarrierRateChangeLog(TenantModelBase):
    """承运价规则变更日志（每次 update/disable/enable/delete/create 都写一条）"""

    __tablename__ = "biz_carrier_rate_change_log"
    __table_args__ = (
        Index("idx_crcl2_rate", "rate_id"),
        Index("idx_crcl2_contract", "contract_id"),
        {"comment": "承运价规则变更日志表"},
    )
    __table_tier__ = "business"

    rate_id: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="承运价规则ID"
    )
    contract_id: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="所属合同ID"
    )
