...
export async function listRoutes(params?: RouteParam) {
  const res = await request.get<ApiResult<Route[]>>('/resource/route/list', {
    params
  });
...
export async function addRoute(data: Route) {
  const res = await request.post<ApiResult<unknown>>('/resource/route', data);
  if (res.data.code === 0) {
...
export async function removeRoute(id: number) {
  const res = await request.delete<ApiResult<unknown>>(`/resource/route/${id}`);
  if (res.data.code === 0) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
