...
<script lang="ts" setup>
  ...
  import {
    getSocialFundAccount,
    listSocialFundTransactions,
    postSocialFundTransaction,
    toggleSocialFundAccountStatus
  ...
  import {
    ...
    fundBizTypeLabel,
    resolveManualSign
  } from '@/views/capacity/self-capacity/driver/fund-account.constants';

  const { hasPermission } = usePermission();
  const canPost = computed(() =>
    hasPermission('capacity:social_capacity:list:fund-post')
  );
  const canFreeze = computed(() =>
    hasPermission('capacity:social_capacity:list:fund-freeze')
  );

  const props = defineProps<{
    visible: boolean;
    capacity: SocialCapacityListItem | null;
  }>();
  ...
  const balanceHint = computed(() => {
    if (balanceNum.value > 0) return '公司欠运力（可结算给运力）';
    if (balanceNum.value < 0) return '运力欠公司（预付未核销）';
    return '两清';
  });
  ...
  const loadAccount = async () => {
    if (!props.capacity?.id) return;
    account.value = await getSocialFundAccount(props.capacity.id);
  };

  const loadTransactions = async (p?: number) => {
    if (!props.capacity?.id) return;
    if (p) page.value = p;
    const res = await listSocialFundTransactions(props.capacity.id, {
      page: page.value,
      limit: pageSize,
  ...
  const toggleStatus = async (status: number) => {
    ...
    try {
      account.value = await toggleSocialFundAccountStatus(
        account.value.id,
        status
      );
  ...
  const submitPost = async () => {
    ...
    posting.value = true;
    try {
      await postSocialFundTransaction(props.capacity.id, {
