...
<script lang="ts" setup>
  ...
  import { pageRoutes, removeRoute } from '@/api/resource/route';
  ...
  const columns = ref<Columns>([
    ...
    { prop: 'routeCode', label: '线路编码', minWidth: 110 },
    { prop: 'origin', label: '起点', minWidth: 100 },
    { prop: 'destination', label: '终点', minWidth: 100 },
    {
      prop: 'distance',
      label: '里程(km)',
      minWidth: 100,
  ...
  const datasource: DatasourceFunction = async ({ page, limit }) => {
    const res = await pageRoutes({ ...where, page, limit });
