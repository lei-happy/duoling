<template>
  <ele-page class="contract-detail-page">
      ...
      <div class="cd-hero">
        <div class="cd-hero__main">
          ...
          <div class="cd-hero__meta">
            ...
            <span class="cd-chip">
              <span class="cd-chip__k">承运商</span>
              <span class="cd-chip__v">{{ contract.carrierName || '—' }}</span>
            </span>
        ...
        >
          ...
          <template #action="{ row }">
            <div
              ...
              :key="`carrier-rate-actions-${row.id}-${row.status ?? ''}`"
            >
              <btn-items
                divider
                type="link"
    ...
    <freight-preview
      ...
      :carrier-id="contract?.carrierId"
      :carrier-name="contract?.carrierName"
    />
  </ele-page>
</template>
...
<script lang="ts" setup>
  ...
  import FreightPreview from './components/freight-preview.vue';
  import {
    getContract,
    listRates,
    removeRate,
  ...
  defineOptions({ name: 'BillingCarrierContractDetailPage' });

  const CONTRACT_LIST_PATH = '/billing/carrier-contract';
  ...
  const contract = ref<CarrierContract | null>(null);
  const ratesCache = ref<CarrierRate[]>([]);
  const pageLoading = ref(false);
  const loadError = ref<string | null>(null);

  const rateTableRef = ref<InstanceType<typeof EleProTable> | null>(null);
  const rateEditVisible = ref(false);
  const rateEditData = ref<CarrierRate | null>(null);
  ...
  const openVersionHistory = (row: CarrierRate) => {
    if (!row?.id) return;
    versionHistoryRateId.value = row.id;
    versionHistoryVisible.value = true;
  };

  const recalcAffected = (row: CarrierRate) => {
    if (!row?.id) return;
    ElMessageBox.confirm(
      '将查找受该承运价规则影响的任务并入队批量重算，确定继续？',
      '重算受影响任务',
      { type: 'warning' }
    )
      .then(async () => {
        ...
        try {
          ...
          loading.close();
          EleMessage.success({
            message: `已入队，影响 ${data?.affectedTaskCount ?? 0} 个任务，新增任务 ${data?.enqueuedTaskCount ?? 0} 条`,
            plain: true
          });
        } catch (e: any) {
          loading.close();
  ...
  const ratesTableCacheKey = computed(
    () => `BillingCarrierContractRateTable-${contract.value?.id ?? '0'}`
  );
  ...
  const ratesDatasource: DatasourceFunction = async ({ pages, where }) => {
    const filtered = filterRates(
      ratesCache.value,
      where as CarrierRateFilterParam | undefined
    );
  ...
  const rateColumns = ref<Columns>([
    ...
    },
    {
      ...
      formatter: (row: CarrierRate) => formatRateBrandModel(row)
    },
    {
      columnKey: 'unitPrice',
      prop: 'unitPrice',
      ...
      align: 'right',
      slot: 'unitPrice',
      formatter: (row: CarrierRate) => formatRateUnitPriceText(row)
    },
    {
      prop: 'priceType',
      label: '运价类型',
    ...
    },
    {
      ...
      width: 64,
      align: 'center',
      formatter: (row: CarrierRate) => (row.isBidirectional === 1 ? '是' : '否')
    },
    {
      prop: 'minAmount',
      label: '最低运费',
  ...
  const rateUnitSuffix = (row: CarrierRate) =>
    row.billingMode === 1
      ? '元/台·km'
      : row.billingMode === 2
        ? '元/单'
  ...
  const formatRateUnitPriceText = (row: CarrierRate) => {
  ...
  const formatRateBrandModel = (row: CarrierRate) => {
  ...
  const rateActionItems = (row: CarrierRate): ButtonItem[] => {
  ...
  const reloadRatesTable = (where?: CarrierRateFilterParam, page?: number) => {
  ...
  const openRateEdit = (row?: CarrierRate) => {
    if (!contract.value?.id) return;
    rateEditData.value = row ?? null;
    rateEditVisible.value = true;
  };

  const removeRateRow = (row: CarrierRate) => {
    ElMessageBox.confirm('确定要删除该承运价明细吗?', '系统提示', {
      type: 'warning',
      draggable: true
    })
      .then(() => {
