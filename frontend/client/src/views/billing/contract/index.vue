      ...
      >
        ...
        <template #action="{ row }">
          <div
            ...
            :key="`contract-actions-${row.id}-${row.status ?? ''}`"
          >
            <btn-items
              divider
              type="link"
...
<script lang="ts" setup>
  ...
  const actionItems = (row: FreightContract): ButtonItem[] => {
    ...
    const rowNorm: FreightContract = { ...row, status: st ?? row.status };
  ...
  const reload = (where?: FreightContractParam, page?: number) => {
    ...
    const opt: { where?: FreightContractParam; page?: number } = {};
  ...
  const resume = (row: FreightContract) => {
    ElMessageBox.confirm(
      `确定将合同「${row.contractName}」恢复为生效吗？恢复后将重新参与运费匹配（仍受合同有效期约束）。`,
      '系统提示',
      { type: 'info', draggable: true }
    )
      .then(() => {
