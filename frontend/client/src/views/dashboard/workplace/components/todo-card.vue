<!-- 我的待办 -->
<template>
  <ele-card :body-style="{ padding: '6px 0', height: '400px' }" class="todo-card">
    <template #header>
    ...
    <template #extra>
      ...
      <div class="todo-card-extra-inner" @mousedown.stop>
        <div class="todo-tabs">
          <!-- 状态筛选 -->
          <el-radio-group id="todo-status-filter" v-model="activeStatus" size="small" @change="handleStatusChange" class="status-radio-group">
            <el-radio-button 
        ...
        <more-icon :hide-edit="true" :hide-remove="true" @command="handleCommand" />
      </div>
    </template>
    ...
    <el-scrollbar :view-style="{ padding: '0px 20px 0 20px' }">
      ...
      <div v-else class="task-list">
        <div 
          ...
          :key="task.id" 
          class="task-item"
          :class="{ 'completed': task.status === 2, 'in-progress': task.status === 1 }"
          @click="editTask(task)"
        >
          <!-- 复选框 -->
          ...
          <div class="task-content">
            ...
            <div class="task-title-row">
              <div class="task-title" :class="{ 'completed-title': task.status === 2 }">
                {{ task.title }}
              </div>
              <!-- 更多操作菜单 - 右上角 -->
              <div class="task-more-action" @click.stop>
                <el-dropdown trigger="click" @command="(cmd) => handleTaskAction(task, cmd)">
                  <el-button link size="small" :icon="More" class="action-btn" />
                  <template #dropdown>
                    <el-dropdown-menu>
            ...
            <div class="task-meta-row">
              <div class="task-meta">
                ...
                <span 
                  ...
                  :class="['priority-indicator', getPriorityClass(task.priority)]"
                >
                ...
                <!-- 已完成任务显示创建时间和完成时间 -->
                <span class="meta-item" v-if="task.status === 2 && task.create_time">
                ...
                <span class="meta-item" v-if="task.status === 2 && task.completed_time">
                  <el-icon class="meta-icon"><Clock /></el-icon>
                ...
                <!-- 非已完成任务显示截止时间 -->
                <span class="meta-item" v-if="task.status !== 2 && task.due_time">
                  <el-icon class="meta-icon"><Clock /></el-icon>
                  <span class="meta-text" :class="{ 'overdue': isOverdue(task.due_time) }">
    ...
    >
      <el-form :model="createForm" :rules="createRules" ref="createFormRef" label-width="90px">
        ...
        <el-form-item label="优先级">
          <el-radio-group id="create-task-priority" v-model="createForm.priority" class="priority-radio-group">
            <el-radio 
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
    >
      <el-form :model="editForm" :rules="createRules" ref="editFormRef" label-width="90px">
        <el-form-item label="任务标题" prop="title" required>
          <el-input 
            id="edit-task-title"
        ...
        <el-form-item label="优先级">
          <el-radio-group id="edit-task-priority" v-model="editForm.priority" class="priority-radio-group">
            <el-radio 
              v-for="item in priorityOptions" 
              :key="item.value"
        ...
        <el-form-item label="状态">
          <el-radio-group id="edit-task-status" v-model="editForm.status" class="status-radio-group">
            <el-radio 
...
<script lang="ts" setup>
  ...
  const getDisabledMinutes = (selectedHour: number) => {
    ...
    if (selectedDateStr) {
      const selectedDate = new Date(selectedDateStr);
      if (selectedDate.toDateString() === today && selectedHour === now.getHours()) {
