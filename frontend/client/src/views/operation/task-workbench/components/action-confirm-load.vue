...
<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="本次装车" prop="itemIds" required>
        <div style="width: 100%">
          <el-table
            ref="itemTableRef"
            :data="loadableItems"
          ...
          <div v-if="!loadableItems.length" class="ele-text-secondary mt-8">
    ...
    <el-divider content-position="left">历史装车记录</el-divider>
    <el-table
      :data="loadHistory"
      size="small"
      border
...
<script lang="ts" setup>
  ...
  const rules: FormRules = {
    ...
    itemIds: [
      {
        validator: (_r, _v, cb) => {
          if (!form.itemIds.length) return cb(new Error('请勾选本次装车的挂接行'));
          cb();
        },
        trigger: 'change'
      }
  ...
  const onOpen = async () => {
    ...
    try {
      ...
      items.value = its;
      loadHistory.value = history.filter((r) => r.eventType === 1);
