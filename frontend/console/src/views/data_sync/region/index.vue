      ...
      <template #header>
        <div class="card-head">
          ...
          <span class="card-head__hint">
            从高德 Web 服务 API 拉取省/市/区县/街道四级数据及中心点坐标，全量写入平台库
            sys_regions。同步过程约 15~20 分钟，请勿重复触发。
          </span>
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  import {
    ...
    triggerRegionSync,
    getRegionSyncJob
  } from '@/api/ops/region-sync';
  import type { RegionSyncJob } from '@/api/ops/region-sync/model';

  defineOptions({ name: 'DataSyncRegion' });

  const tableRef = ref<InstanceType<typeof EleProTable> | null>(null);
  const maxConcurrent = ref(2);
  const requestDelayMs = ref(350);
  ...
  const columns = ref<Columns>([
    ...
    },
    {
      ...
      align: 'center',
      formatter: (row) =>
        row.totalCount != null ? String(row.totalCount) : '—'
    },
  ...
  const onTrigger = async () => {
    try {
      await ElMessageBox.confirm(
        '将从高德 API 全量同步省/市/区县/街道数据到平台库，过程较长且会软删旧数据后覆盖写入。确认继续？',
        '行政区域同步',
  ...
  const openLog = async (row: RegionSyncJob) => {
    try {
      const latest = await getRegionSyncJob(row.jobId);
      activeLog.value =
        latest.logText || latest.errorMessage || '（无日志）';
