 ...
 * 获取地区子级列表（懒加载树）
 */
export async function getRegionTree(pcode?: number | null) {
 ...
 * 获取地区详情
 */
export async function getRegion(code: number) {
  const res = await request.get<ApiResult<Region>>(
    `/basic-data/region/${code}`
  );
  if (res.data.code === 0) {
    return res.data.data;
  }
 ...
 * 删除地区
 */
export async function removeRegion(code: number) {
